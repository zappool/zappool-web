...
Yes. We use __5 sats__ as the minimum payout amount. If that amount is not reached, it will be accumulated.
...
- For username enter your lightning address, with "LNAD:" in front and all '.' dots changed to '_' underscores -- ex. `LNAD:something@walletofsatoshi_com`
...
You can see your own stats if you log in (with your npub).
...
The recommended simple way is to use your 'regular' Nostr npub. However, it will be visible that you are mining on Zappool (your payout zaps, etc.). For most, you should be proud of this, not hide it!
...
Ocean uses a relatively large share difficulty (128k). Modern industrial miners can solve this difficulty in seconds, but for a lower-hashrate Bitaxe it takes minutes. The time has a wide variation due to luck, it can be as low as less than a minute, but it can be as high as 30 minutes. This, however does not affect the overall performance of the miner.
...
### Can I get paid to my Nostr account without the Zap event?

If you want the payouts to your Nostr lightning address without the Zap events, you can:

- Prefix your username by "NOLN:".
Example: `NOLN:npub17qt0gf99uxe3n3aa8nj93hspe0x3vfn7wwmv42adfc96y8g8ngcqpakg7l` (`NOLN` is short for Nostr Lightning);
- or [not implemented] Set the desired payment method in your nostr-user-setting to "Nostr Lightning Address" `NOLN`.


### Can I get paid to a lightning address without any Nostr hoopla?
...
- it has to be prefixed with "LNAD:" (5 characters)

Example: `LNAD:zappool@blink_sv` or `LNAD:zappool@blink_sv.device`.
...
In solo mining most miners get nothing most of the time, but in the very unlikely case someone finds a block, he gets the whole block reward.
In pooled mining (used by most miners), miners share the rewards proportionally to their hash power.
...
### Can I donate to someone else using Zappool?

Absolutely! You can donate the earnings from your miner to another Nostr user easily (and without permission). Just set the npub of the target user account as user ID in yout miner.

Consider giving back to the creators of Bitaxe, Nostr, Lightning, etc.
...
Zappool is currently live (as of Jan'26). The beta version launched in October 2025.
...
Yes, the software can be found here:
[github.com/zappool](https://github.com/zappool).

It includes the modified Ocean datum service, the backend components, and the web page.
...
### Where is the Zappool server located?

The pool server is located in North America, Eastern side. In the future we may offer servers in other regions (e.g. Europe), for lower latency (if justified by usage).
...
No, not to my knowledge. The chances are currently astronomically low, but not zero!
...
For _solo_ mining, two popular pools are:
...
### Can I make even with my Bitaxe (using Zappool)?

Short answer: no. Even with pooled mining, you will not be able to recover the cost of your Bitaxe
You may earn enough to recover the cost of the electricity used (depending on your power price).
...
So forget about profits, relax, and zap away your earned sats!


### What is a Proxy Pool?

A proxy pool is a pool, but it uses another pool -- the 'mother pool' -- as a regular mining client: it submits work shares and receives earnings. To its clients the proxy pool appears as a mining pool: it provides work, accepts shares, and distributes earnings.
...
Why is it paying less than Braiins?
